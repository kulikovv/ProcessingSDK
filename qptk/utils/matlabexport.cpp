...
MatlabExport::MatlabExport(QObject *parent) :
    RawDataExport(parent){_extension="mat";}
MatlabExport::~MatlabExport(){}

void MatlabExport::saveData(QString name, cv::Mat img,QString varname)
{
    writeMat(img,QString(name+"."+_extension).toStdString().c_str(),varname.toStdString().c_str());
}

void MatlabExport::writeMat( cv::Mat const& mat, const char* filename, const char* varName, bool bgr2rgb )
{
   int textLen = 116;
   char* text;
