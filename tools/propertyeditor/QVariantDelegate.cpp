    ...
    : QItemDelegate(parent)
{
	m_finishedMapper = new QSignalMapper(this);
        QObject::connect(m_finishedMapper, SIGNAL(mapped(QWidget*)), this, SIGNAL(commitData(QWidget*)));
        QObject::connect(m_finishedMapper, SIGNAL(mapped(QWidget*)), this, SIGNAL(closeEditor(QWidget*)));
}
		...
		editor = p->createEditor(parent, option);
                if (editor) {
                        if (editor->metaObject()->indexOfSignal("editFinished()") != -1) {
                                QObject::connect(editor, SIGNAL(editFinished()), m_finishedMapper, SLOT(map()));
