...
QVariant QPropertyModel::data ( const QModelIndex & index, int role /*= Qt::DisplayRole*/ ) const
{
	...
	{
        case Qt::ToolTipRole:
            if(!item->editorHints().isEmpty())
            {
                return item->editorHints();
            }
...
void QPropertyModel::addItems(QList<QObject*> propertyObject){
    // first create property <-> class hierarchy
    QList<PropertyPair> propertyMap;
    QList<QObject*> finalClassList;
    foreach(QObject* obj,propertyObject){
    ...
    foreach(QObject* obj,finalClassList){
       ...
       // Create Property Item for class node
        ...
        {
                QMetaProperty property = metaObject->property(j);
                if(property.isUser())
                {
                    qDebug() << obj->objectName() << QString(property.name());
                    Property* p = 0;
                    if (property.type() == QVariant::UserType && !m_userCallbacks.isEmpty())
                    {
                        ...
                        {
                            p = (*iter)(property.name(), obj, propertyItem);
                            ++iter;
                        }
                    }
                    if( p == 0){
                        if(property.isEnumType()){
                            p = new EnumProperty(property.name(), obj, propertyItem);
                        } else {
                            p = new Property(property.name(), obj, propertyItem);
                        }
                    }
        ...
        if( propertyItem )	addDynamicProperties( propertyItem, obj );
    }
    endInsertRows();
}
