...
void Property::setValue()
{
    if (m_propertyObject)
            m_propertyObject->setProperty(qPrintable(objectName()), QLine());
}
...
QWidget* Property::createEditor(QWidget *parent, const QStyleOptionViewItem& /*option*/)
{
		...
		editor->setProperty("maximum", INT_MAX);
                QObject::connect(editor, SIGNAL(valueChanged(int)), this, SLOT(setValue(int)));
		break;
        case (int)QMetaType::QLine:
                 editor = new QPushButton(parent);
                 editor->setProperty("text", m_hints);
                 QObject::connect(editor, SIGNAL(clicked()), this, SLOT(setValue()));
                 break;
		...
		editor->setProperty("minimum", -INT_MAX);
		editor->setProperty("maximum", INT_MAX);
        editor->setProperty("decimals",6);
                QObject::connect(editor, SIGNAL(valueChanged(double)), this, SLOT(setValue(double)));
                break;
        case (int)QVariant::StringList:
                editor = new QComboBox(parent);
               // ceditor->addItems(value.toStringList());
                //editor=ceditor;
               // QComboBox* ceditor
                {
                    QComboBox* ceditor = (QComboBox*)editor;
                    QVariant var = value();
                    ceditor->addItems(var.toStringList());
                    QObject::connect(ceditor, SIGNAL(currentIndexChanged(int)), this, SLOT(setValue(int)));
                }
            break;
	default:
		return editor;
